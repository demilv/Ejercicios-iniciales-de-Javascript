...
for (const usuarios of users){
    for (const volumenes in usuarios.favoritesSounds){             
        if(sonidos.includes(volumenes)){
        const localizacion = sonidos.indexOf(volumenes)
        repeticiones[localizacion]++              
        }else{
        sonidos.push(volumenes)
        repeticiones.push(1)
        }
    }
}
...
for (i = 0; i<sonidos.length; i++){
    console.log(`La cantidad de veces que se repite el sonido ${sonidos[i]} es de ${repeticiones[i]}`)
}
